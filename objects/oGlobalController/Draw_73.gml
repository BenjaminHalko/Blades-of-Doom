...
draw_text_transformed(room_width/2,19,string(floor(oGameManager.time div 60))+":"+string_replace(string_format(oGameManager.time % 60,2,2)," ","0"),timePulse*0.5+1,timePulse*0.5+1,0);
...
for(var j = 0; j < 2; j++) {
	...
	if _health <= 0 and oGameManager.players[1] != noone draw_text(_x,15,string(floor(oGameManager.scores[j] div 60))+":"+string_replace(string_format(oGameManager.scores[j] % 60,2,2)," ","0"));
	else {
		for(var i = 0; i < 3; i++) {
			var _pulse = 1;
			var _col = c_white;
